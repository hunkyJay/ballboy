...
public class GameWindow {
    ...
    private boolean isGameOver;//Add a flag to check whether the game is finished
    //private final GraphicsContext gc;//Add a GraphicsContext to draw

    private InformationDisplayer gameVictoryDisplay;//Winner page displayer
    ...
    public GameWindow(
        ...
        isGameOver = false;
        //Elements added to window
        gameVictoryDisplay = new GameVictoryDisplay();
//        Canvas canvas = new Canvas(width,height);
//        gc = canvas.getGraphicsContext2D();
//        pane.getChildren().add(canvas);

        Text totalScore = new Text(10, 15, "Total Score: 0");
        Text levelScore = new Text(10, 30, "Current level Score: 0");
        Text rScore = new Text(10, 45, "Current R: 0");
        Text gScore = new Text(10, 60, "Current G: 0");
        Text bScore = new Text(10, 75, "Current B: 0");
        pane.getChildren().addAll(totalScore, levelScore, rScore, gScore, bScore);
    }
    ...
    private void draw() {
        if(model.isGameFinished()) {
            //Passed all levels, win the game
            if(!isGameOver) {
                isGameOver = true;
                gameVictoryDisplay.display(this);
            }
        }else {
            model.notifyObservers();//update scores
