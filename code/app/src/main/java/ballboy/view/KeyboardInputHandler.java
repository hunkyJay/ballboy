...
import ballboy.model.memento.MementoStore;
...
class KeyboardInputHandler {
    ...
    private final MementoStore mementoStore = new MementoStore();//Using keyboard to handle memento mode
...
//        sounds.put("jump", mediaPlayer);
    ...
    void handlePressed(KeyEvent keyEvent) {
        ...
        if (keyEvent.getCode().equals(KeyCode.S)) {
            mementoStore.setMemento(model.save());
        }
        if (keyEvent.getCode().equals(KeyCode.Q)) {
            model.load(mementoStore.getMemento());
        }
